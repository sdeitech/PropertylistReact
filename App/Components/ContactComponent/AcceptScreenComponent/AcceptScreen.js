...
class AcceptScreen extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (response !== undefined) {
      if (response.code === Strings.STATUS_OK) {
        ...
        this.setState({
          defaultContactList: response.data,
  ...
  renderList(item, index) {
    ...
    let receiverId = item.participants.length!=0?item.participants[0]:"";
    let name =item.participants.length!=0?item.participants[0].firstname:"" + " " + item.participants.length!=0?item.participants[0].lastname:"";
    ...
    if (userType === "Group") {
      ...
      name =item.participants.length!=0?item.participants[0].firstname:"" + " " + item.participants.length!=0?item.participants[0].lastname:"";
    }
      ...
      >
        ...
        <View style={AcceptScreenStyle.cardContainer}>
          <TouchableOpacity
            ...
            onPress={() =>
              Actions.chatScreen({
                seller: userType === "Admin" ? "Contact Admin" : name,
      ...
      <View style={{ flex: 1 }}>
        {this.state.isSpinnerVisible ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            {this.state.acceptedContactList.length != 0 ? (
              <FlatList
